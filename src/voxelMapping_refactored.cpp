...
// [VoxelMap] 引入重构后的 VoxelMap 工具头，包含以下核心类型和函数：
//   数据结构：VoxelLocation, OctoTree, PointWithCovariance, PointToPlaneMatch, Plane
//   API：BuildVoxelMap, UpdateVoxelMap, TransformLidar, CalcBodyCovariance,
//        BuildResidualListOMP, BuildResidualListNormal, PubVoxelMap, PubPlaneMap
#include "voxel_map_util_refactored.hpp" // note: refine
#include <Eigen/Core>
...
// note: refine — pointWithCov → PointWithCovariance
const bool var_contrast(PointWithCovariance &x, PointWithCovariance &y) {
...
// note: refine — ptpl → PointToPlaneMatch
void publish_effect_world(const ros::Publisher &pubLaserCloudEffect,
                          const ros::Publisher &pubPointWithCov,
                          const std::vector<PointToPlaneMatch> &ptpl_list) {
...
void publish_effect(const ros::Publisher &pubLaserCloudEffect) {
  ...
  for (int i = 0; i < effct_feat_num; i++) {
    ...
    v = 1.0 - v;
    uint8_t r, g, b;
    MapJet(v, 0, 1, r, g, b); // note: refine — mapJet → MapJet
    pi.r = r;
    pi.g = g;
    pi.b = b;
    effect_cloud_world->points.push_back(pi);
...
int main(int argc, char **argv) {
  ...
  bool init_map = false;
  // [VoxelMap] 核心地图数据结构：
  //   key   = VoxelLocation (体素的三维整数坐标)
  //   value = OctoTree*    (每个体素的八叉树，负责维护点云及拟合平面)
  // note: refine — VOXEL_LOC → VoxelLocation
  std::unordered_map<VoxelLocation, OctoTree *> voxel_map;
  ...
  while (status) {
    ...
    if (sync_packages(Measures)) {
      ...
      flg_EKF_inited = (Measures.lidar_beg_time - first_lidar_time) < INIT_TIME
                           ? false
                           : true;
      // [VoxelMap] ===== Phase 1: 地图初始化（仅首帧执行一次）=====
      // 流程：TransformLidar → 逐点 CalcBodyCovariance → BuildVoxelMap
      if (flg_EKF_inited && !init_map) {
        pcl::PointCloud<pcl::PointXYZI>::Ptr world_lidar(
            new pcl::PointCloud<pcl::PointXYZI>);
        Eigen::Quaterniond q(state.rot_end);
        // [VoxelMap] API: TransformLidar — 将原始点云从 body 系变换到 world 系
        TransformLidar(state, p_imu, feats_undistort, world_lidar); // note: refine — transformLidar → TransformLidar
        // [VoxelMap] 构建 PointWithCovariance 列表，每个点携带其在 world 系下的协方差
        std::vector<PointWithCovariance> pv_list; // note: refine — pointWithCov → PointWithCovariance
        for (size_t i = 0; i < world_lidar->size(); i++) {
          PointWithCovariance pv; // note: refine — pointWithCov → PointWithCovariance
          ...
          // if z=0, error will occur in CalcBodyCovariance. To be solved
          if (point_this[2] == 0) {
            point_this[2] = 0.001;
          }
          M3D cov;
          // [VoxelMap] API: CalcBodyCovariance — 根据测距误差(ranging_cov)和角度误差(angle_cov)
          //   在 body 系下建立点的协方差矩阵，随后旋转到 world 系并叠加状态估计不确定度
          CalcBodyCovariance(point_this, ranging_cov, angle_cov, cov); // note: refine — calcBodyCov → CalcBodyCovariance
        ...
        // todo: 初始化地图
        // [VoxelMap] API: BuildVoxelMap — 将 pv_list 中所有点分配到对应体素，
        //   对每个体素构建 OctoTree 并触发平面拟合（InitOctoTree）
        //   参数说明：voxel_size=体素边长, max_layer=树最大层数,
        //             layer_point_size=各层触发平面拟合的最少点数,
        //             min_eigen_value=平面判断阈值（最小特征值）
        BuildVoxelMap(pv_list, max_voxel_size, max_layer, layer_size, // note: refine — buildVoxelMap → BuildVoxelMap
        ...
        if (publish_voxel_map) {
          // todo: 发布地图
          // [VoxelMap] API: PubVoxelMap — 遍历地图中已更新的平面，以 Cylinder Marker 形式发布可视化
          PubVoxelMap(voxel_map, publish_max_voxel_layer, voxel_map_pub); // note: refine — pubVoxelMap → PubVoxelMap
        }
        init_map = true;
        continue;
      }
      ...
      for (iterCount = 0; iterCount < NUM_MAX_ITERATIONS; iterCount++) {
        ...
        // [VoxelMap] ===== Phase 2: 扫描匹配（每次 EKF 迭代执行）=====
        // 流程：TransformLidar → 构建带协方差点列表 pv_list → BuildResidualListOMP
        //       → 利用 plane_cov 计算 R_inv → 构建 Jacobian Hsub
        std::vector<PointToPlaneMatch> ptpl_list; // note: refine — ptpl → PointToPlaneMatch
        ...
        pcl::PointCloud<pcl::PointXYZI>::Ptr world_lidar(
            new pcl::PointCloud<pcl::PointXYZI>);
        // [VoxelMap] API: TransformLidar — 用当前状态估计将下采样点云变换到 world 系
        TransformLidar(state, p_imu, feats_down_body, world_lidar); // note: refine — transformLidar → TransformLidar
        // [VoxelMap] 构建带 world 系协方差的点列表（叠加旋转不确定度 rot_var 和平移不确定度 t_var）
        for (size_t i = 0; i < feats_down_body->size(); i++) {
          PointWithCovariance pv; // note: refine — pointWithCov → PointWithCovariance
        ...
        // [VoxelMap] API: BuildResidualListOMP — OpenMP 并行版扫描匹配
        //   对每个点在 voxel_map 中查找最近平面（支持跨体素回退搜索）
        //   输出：ptpl_list（点-面匹配对，含 normal/center/plane_cov/d）
        //         non_match_list（未找到匹配平面的点）
        //   匹配条件：点到平面距离 < sigma_num * sqrt(sigma_l)，其中
        //     sigma_l = J_nq * plane_cov * J_nq^T + n^T * point_cov * n
        ...
        for (int i = 0; i < effct_feat_num; i++) {
          ...
          M3D cov;
          // [VoxelMap] API: CalcBodyCovariance — 重新计算点的 body 系协方差（用于 EKF 观测噪声）
          if (calib_laser) {
            CalcBodyCovariance(point_this, ranging_cov, CALIB_ANGLE_COV, cov); // note: refine — calcBodyCov → CalcBodyCovariance
          } else {
            CalcBodyCovariance(point_this, ranging_cov, angle_cov, cov); // note: refine — calcBodyCov → CalcBodyCovariance
          }
          ...
          // [VoxelMap] 利用 PointToPlaneMatch.plane_cov（平面协方差）计算观测噪声 sigma_l
          //   sigma_l = J_nq * plane_cov * J_nq^T + n^T * point_cov * n
          //   R_inv(i) = 1 / sigma_l，作为 EKF 的信息矩阵权重
          Eigen::Matrix<double, 1, 6> j_nq; // note: refine — J_nq → j_nq
      ...
      // [VoxelMap] ===== Phase 3: 地图增量更新 =====
      // 流程：TransformLidar（用收敛后的最终状态）→ 重建 pv_list → var_contrast 排序 → UpdateVoxelMap
      // 注意：此处 pv_list 使用 EKF 收敛后的 state.rot_end/pos_end，精度高于匹配阶段
      /*** add the  points to the voxel map ***/
      auto map_incremental_start = std::chrono::high_resolution_clock::now();
      pcl::PointCloud<pcl::PointXYZI>::Ptr world_lidar(
          new pcl::PointCloud<pcl::PointXYZI>);
      // [VoxelMap] API: TransformLidar — 用 EKF 收敛后的最终状态将点云变换到 world 系
      TransformLidar(state, p_imu, feats_down_body, world_lidar); // note: refine — transformLidar → TransformLidar
      // [VoxelMap] 按协方差范数从小到大排序（var_contrast），优先插入高质量点以稳定平面拟合
      std::vector<PointWithCovariance> pv_list; // note: refine — pointWithCov → PointWithCovariance
      for (size_t i = 0; i < world_lidar->size(); i++) {
        PointWithCovariance pv; // note: refine — pointWithCov → PointWithCovariance
        pv.point << world_lidar->points[i].x, world_lidar->points[i].y,
      ...
      // [VoxelMap] API: UpdateVoxelMap — 将 pv_list 中的点逐一分配到对应体素的 OctoTree
      //   内部触发：OctoTree::UpdateOctoTree → 点数超阈值后重新 InitPlane 或 UpdatePlane
      //   当点数达到 max_cov_points_size：停止更新协方差；达到 max_points_size：停止插入新点
      UpdateVoxelMap(pv_list, max_voxel_size, max_layer, layer_size, // note: refine — updateVoxelMap → UpdateVoxelMap
                     max_points_size, max_points_size, min_eigen_value,
                     voxel_map);
      ...
      if (publish_voxel_map) {
        // [VoxelMap] API: PubVoxelMap — 发布地图中本帧新增/更新的平面（仅 is_update=true 的平面）
        PubVoxelMap(voxel_map, publish_max_voxel_layer, voxel_map_pub); // note: refine — pubVoxelMap → PubVoxelMap
      }
