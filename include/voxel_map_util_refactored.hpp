#ifndef VOXEL_MAP_UTIL_REFACTORED_HPP
#define VOXEL_MAP_UTIL_REFACTORED_HPP
...
struct VoxelLocation {
  ...
  VoxelLocation(int64_t vx = 0, int64_t vy = 0, int64_t vz = 0)
      : x(vx), y(vy), z(vz) {}
...
class OctoTree {
 public:
  std::vector<PointWithCovariance> temp_points_;
  std::vector<PointWithCovariance> new_points_;
  Plane *plane_ptr_;
  int max_layer_;
  bool indoor_mode_;
  int layer_;
  ...
  void InitPlane(const std::vector<PointWithCovariance> &points, Plane *plane) {
    ...
    j_q << 1.0 / plane->points_size, 0, 0, 0, 1.0 / plane->points_size, 0, 0,
        0, 1.0 / plane->points_size;
    ...
    if (evals_real(evals_min) < planer_threshold_) {
      ...
      plane->normal << evecs.real()(0, evals_min),
          evecs.real()(1, evals_min), evecs.real()(2, evals_min);
      plane->y_normal << evecs.real()(0, evals_mid),
          evecs.real()(1, evals_mid), evecs.real()(2, evals_mid);
      plane->x_normal << evecs.real()(0, evals_max),
          evecs.real()(1, evals_max), evecs.real()(2, evals_max);
  ...
  void UpdatePlane(const std::vector<PointWithCovariance> &points,
                   Plane *plane) {
  ...
  void CutOctoTree() {
    ...
    for (size_t i = 0; i < temp_points_.size(); i++) {
      ...
      if (leaves_[leaf_num] == nullptr) {
        leaves_[leaf_num] = new OctoTree(
            max_layer_, layer_ + 1, layer_point_size_, max_points_size_,
            max_cov_points_size_, planer_threshold_);
        leaves_[leaf_num]->voxel_center_[0] =
            voxel_center_[0] + (2 * xyz[0] - 1) * quater_length_;
        leaves_[leaf_num]->voxel_center_[1] =
            voxel_center_[1] + (2 * xyz[1] - 1) * quater_length_;
        leaves_[leaf_num]->voxel_center_[2] =
            voxel_center_[2] + (2 * xyz[2] - 1) * quater_length_;
        leaves_[leaf_num]->quater_length_ = quater_length_ / 2;
      }
      leaves_[leaf_num]->temp_points_.push_back(temp_points_[i]);
      leaves_[leaf_num]->new_points_num_++;
    }
  ...
  void UpdateOctoTree(const PointWithCovariance &pv) {
    if (!init_octo_) {
      new_points_num_++;
      all_points_num_++;
      ...
      if (plane_ptr_->is_plane) {
        if (update_enable_) {
          ...
          if (all_points_num_ >= max_cov_points_size_) {
            update_cov_enable_ = false;
            std::vector<PointWithCovariance>().swap(temp_points_);
          }
          if (all_points_num_ >= max_points_size_) {
            update_enable_ = false;
            plane_ptr_->update_enable = false;
            std::vector<PointWithCovariance>().swap(new_points_);
          }
        } else {
          return;
        }
        ...
        if (layer_ < max_layer_) {
          ...
          int leaf_num = 4 * xyz[0] + 2 * xyz[1] + xyz[2];
          if (leaves_[leaf_num] != nullptr) {
            leaves_[leaf_num]->UpdateOctoTree(pv);
          } else {
            leaves_[leaf_num] = new OctoTree(
                max_layer_, layer_ + 1, layer_point_size_, max_points_size_,
                max_cov_points_size_, planer_threshold_);
            leaves_[leaf_num]->layer_point_size_ = layer_point_size_;
            leaves_[leaf_num]->voxel_center_[0] =
                voxel_center_[0] + (2 * xyz[0] - 1) * quater_length_;
            leaves_[leaf_num]->voxel_center_[1] =
                voxel_center_[1] + (2 * xyz[1] - 1) * quater_length_;
...
void BuildVoxelMap(
    const std::vector<PointWithCovariance> &input_points,
  ...
  size_t pl_size = input_points.size();
  for (size_t i = 0; i < pl_size; i++) {
    const PointWithCovariance p_v = input_points[i];
    float loc_xyz[3];
    for (int j = 0; j < 3; j++) {
...
void BuildSingleResidual(const PointWithCovariance &pv,
  ...
  if (current_octo->plane_ptr_->is_plane) {
    ...
    if (current_layer < max_layer) {
      for (size_t leaf_num = 0; leaf_num < 8; leaf_num++) {
        if (current_octo->leaves_[leaf_num] != nullptr) {
          OctoTree *leaf_octo = current_octo->leaves_[leaf_num];
...
void BuildResidualListOMP(
  ...
  std::mutex mylock;
  ptpl_list.clear();
  std::vector<PointToPlaneMatch> all_ptpl_list(pv_list.size());
...
#pragma omp parallel for
  ...
  for (int i = 0; i < index.size(); i++) {
    PointWithCovariance pv = pv_list[i];
    float loc_xyz[3];
    for (int j = 0; j < 3; j++) {
...
void CalcVectQuaternion(const Eigen::Vector3d &x_vec,
...
void CalcQuaternion(const Eigen::Vector3d &vec, const int axis,
                    geometry_msgs::Quaternion &q) {
...
void PubNoPlaneMap(
    const std::unordered_map<VoxelLocation, OctoTree *> &feat_map,
...
void PubVoxelMap(
    const std::unordered_map<VoxelLocation, OctoTree *> &voxel_map,
...
void CalcBodyCovariance(Eigen::Vector3d &pb, const float range_inc,
...
#endif // VOXEL_MAP_UTIL_REFACTORED_HPP
